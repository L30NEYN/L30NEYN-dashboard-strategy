 ...
 * @license MIT
 *
 * WICHTIG: Dieses Script ist bewusst KEIN ES-Modul (kein "export" / kein "import").
 * Home Assistant lädt Lovelace-Ressourcen als type="module", was zu einem
 * Timing-Problem führt: HA versucht die Strategy zu nutzen bevor das Modul
 * fertig registriert ist. Durch IIFE-Wrapping wird customElements.define()
 * synchron beim Parsen ausgeführt.
 */
...
(function () {
  ...
  const loadRegistryData = async (hass) => {
    ...
    // Option 1: Gecachte Daten (bevorzugt, schnellste Methode)
  ...
  const L30NEYNHelpers = {
    ...
    filterAvailable(entities) {
      return entities.filter(e =>
        e?.entity_id && !e.disabled_by && !e.hidden_by && !e.entity_category
      );
    },
  ...
  const L30NEYNOverviewView = {
    ...
    _areaCards(areas, hass, entities, devices) {
      const deviceAreaMap = L30NEYNHelpers.buildDeviceAreaMap(devices);
      return (areas || []).filter(a => a && !(a.labels?.includes('no_dboard'))).map(area => {
        const ae = L30NEYNHelpers.filterAvailable(L30NEYNHelpers.filterByLabels(L30NEYNHelpers.filterByArea(entities, area.area_id, deviceAreaMap)));
    ...
    static async generate(config, hass) {
      try {
        ...
        if (registryData.source === 'error') {
          ...
          return {
            views: [{
              title: 'Fehler', path: 'overview', icon: 'mdi:alert-circle',
              cards: [L30NEYNCardBuilders.buildErrorCard(
        ...
        return {
          views: [{
            title: 'Kritischer Fehler', path: 'overview', icon: 'mdi:alert-octagon',
            cards: [L30NEYNCardBuilders.buildErrorCard(
  ...
  try {
    ...
    if (e.name === 'NotSupportedError') {
      console.warn('[L30NEYN] Strategy already registered (NotSupportedError) - skipping');
    } else {
