...
(function () {
  ...
  window.L30NEYNHelpers = {
    filterByLabels(entities) {
      return entities.filter(e => !(e.labels && e.labels.includes('no_dboard')));
    },
    ...
    filterByDomain(entities, domains) {
      ...
      return entities.filter(e => s.has(e.entity_id.split('.')[0]));
    },
    filterAvailable(entities) {
      return entities.filter(e => !e.disabled_by && !e.hidden_by && !e.entity_category);
    },
    filterByConfigHidden(entities, hiddenIds = []) {
      const s = new Set(hiddenIds);
      return entities.filter(e => !s.has(e.entity_id));
    },
    sortByName(entities) {
  ...
  window.L30NEYNDataCollectors = {
    collectLights(hass, entities, config) {
      ...
      let lights = h.filterAvailable(h.filterByLabels(entities.filter(e => e.entity_id.startsWith('light.'))));
      const on = [], off = [];
      lights.forEach(e => { const s = hass.states[e.entity_id]; if (!s) return; s.state === 'on' ? on.push(e.entity_id) : off.push(e.entity_id); });
      return { on, off };
    },
    collectCovers(hass, entities, config) {
      const h = window.L30NEYNHelpers;
      let covers = h.filterAvailable(h.filterByLabels(entities.filter(e => e.entity_id.startsWith('cover.'))));
      const open = [], closed = [];
      covers.forEach(e => { const s = hass.states[e.entity_id]; if (!s) return; s.state === 'open' ? open.push(e.entity_id) : closed.push(e.entity_id); });
      return { open, closed };
    },
    collectSecurity(hass, entities, config) {
      const h = window.L30NEYNHelpers;
      let sec = h.filterAvailable(h.filterByLabels(entities.filter(e => ['lock','binary_sensor','alarm_control_panel','cover'].includes(e.entity_id.split('.')[0]))));
      ...
      sec.forEach(e => {
        ...
        } else if (d === 'cover' && s.attributes.device_class === 'garage') doors.push(e.entity_id);
      });
      return { locks, doors, windows, alarm };
    },
    collectBatteries(hass, entities, config) {
      const h = window.L30NEYNHelpers;
      let bats = h.filterAvailable(h.filterByLabels(entities.filter(e => {
        if (!e.entity_id.startsWith('sensor.')) return false;
        const s = hass.states[e.entity_id]; if (!s) return false;
        return s.attributes.device_class === 'battery' || s.attributes.unit_of_measurement === '%';
      })));
  ...
  window.L30NEYNStatisticsCollectors = {
    collectEnergy(hass, entities, config) {
      const ee = entities.filter(e => {
        const s = hass.states[e.entity_id]; if (!s) return false;
        return s.attributes.device_class === 'energy' || s.attributes.unit_of_measurement === 'kWh' || e.entity_id.includes('energy');
      });
      let totalConsumption = 0, totalProduction = 0;
      const consumers = [], producers = [];
      ee.forEach(e => {
        const s = hass.states[e.entity_id]; if (!s || s.state === 'unavailable' || s.state === 'unknown') return;
        const v = parseFloat(s.state); if (isNaN(v)) return;
        if (e.entity_id.includes('solar') || e.entity_id.includes('production')) {
          totalProduction += v;
          producers.push({ entity_id: e.entity_id, name: s.attributes.friendly_name || e.entity_id, value: v, unit: s.attributes.unit_of_measurement || 'kWh' });
        } else {
          totalConsumption += v;
          consumers.push({ entity_id: e.entity_id, name: s.attributes.friendly_name || e.entity_id, value: v, unit: s.attributes.unit_of_measurement || 'kWh' });
        }
      });
      return { totalConsumption: totalConsumption.toFixed(2), totalProduction: totalProduction.toFixed(2), netConsumption: (totalConsumption - totalProduction).toFixed(2), consumers: consumers.sort((a, b) => b.value - a.value).slice(0, 5), producers: producers.sort((a, b) => b.value - a.value) };
    },
    collectClimate(hass, entities, areas, config) {
      const tempSensors = entities.filter(e => { const s = hass.states[e.entity_id]; if (!s) return false; return s.attributes.device_class === 'temperature' || s.attributes.unit_of_measurement === '°C' || s.attributes.unit_of_measurement === '°F'; });
      const humSensors = entities.filter(e => { const s = hass.states[e.entity_id]; if (!s) return false; return s.attributes.device_class === 'humidity' || s.attributes.unit_of_measurement === '%'; });
      ...
      tempSensors.forEach(e => {
        ...
        const v = parseFloat(s.state); if (isNaN(v)) return;
        tSum += v; tCount++;
        const area = areas.find(a => a.area_id === e.area_id);
        if (area) roomTemps.push({ area: area.name, value: v, unit: s.attributes.unit_of_measurement || '°C' });
      });
      ...
      return { avgTemperature: tCount > 0 ? (tSum / tCount).toFixed(1) : 'N/A', avgHumidity: hCount > 0 ? (hSum / hCount).toFixed(0) : 'N/A', minTemperature: roomTemps.length ? Math.min(...roomTemps.map(r => r.value)).toFixed(1) : 'N/A', maxTemperature: roomTemps.length ? Math.max(...roomTemps.map(r => r.value)).toFixed(1) : 'N/A', roomTemps: roomTemps.sort((a, b) => b.value - a.value), roomHumidity: roomHumidity.sort((a, b) => b.value - a.value) };
    },
    collectSystemHealth(hass, entities, config) {
      const total = Object.keys(hass.states).length;
      let unavail = 0, unknown = 0;
      Object.values(hass.states).forEach(s => { if (s.state === 'unavailable') unavail++; if (s.state === 'unknown') unknown++; });
      const totalDevices = entities.map(e => e.device_id).filter((id, i, s) => id && s.indexOf(id) === i).length;
      const updates = Object.keys(hass.states).filter(id => id.startsWith('update.')).map(id => hass.states[id]).filter(s => s.state === 'on');
      ...
      return { totalEntities: total, totalDevices, unavailableCount: unavail, unknownCount: unknown, healthPercentage: ((total - unavail - unknown) / total * 100).toFixed(1), updatesAvailable: updates.length, updateEntities: updates.map(e => ({ entity_id: e.entity_id, name: e.attributes.friendly_name || e.entity_id, version: e.attributes.latest_version })), automations: { total: automations.length, active: activeAutomations.length } };
    },
    collectNetwork(hass, entities, config) {
      const ne = entities.filter(e => e.entity_id.includes('network') || e.entity_id.includes('bandwidth') || e.entity_id.includes('speedtest') || e.entity_id.includes('ping'));
      const stats = { download: null, upload: null, ping: null };
      ne.forEach(e => {
        const s = hass.states[e.entity_id]; if (!s || s.state === 'unavailable') return;
        if (e.entity_id.includes('download')) stats.download = { value: parseFloat(s.state), unit: s.attributes.unit_of_measurement || 'Mbit/s' };
        else if (e.entity_id.includes('upload')) stats.upload = { value: parseFloat(s.state), unit: s.attributes.unit_of_measurement || 'Mbit/s' };
        else if (e.entity_id.includes('ping')) stats.ping = { value: parseFloat(s.state), unit: s.attributes.unit_of_measurement || 'ms' };
      });
      return stats;
    },
  };
  ...
  window.L30NEYNStatisticsCardBuilders = {
    getTemperatureColor(t) { if (t < 18) return 'blue'; if (t < 22) return 'green'; if (t < 25) return 'orange'; return 'red'; },
    getHumidityColor(h) { if (h < 30) return 'orange'; if (h < 60) return 'green'; if (h < 70) return 'orange'; return 'red'; },
    buildEnergyCard(stats, options = {}) {
      const entities = [{ type: 'custom:mushroom-chips-card', chips: [{ type: 'template', icon: 'mdi:lightning-bolt', content: `${stats.totalConsumption} kWh`, icon_color: 'orange' }, { type: 'template', icon: 'mdi:solar-power', content: `${stats.totalProduction} kWh`, icon_color: 'green' }, { type: 'template', icon: 'mdi:transmission-tower', content: `${stats.netConsumption} kWh`, icon_color: stats.netConsumption > 0 ? 'red' : 'blue' }] }];
      if (stats.consumers.length) { entities.push({ type: 'section', label: 'Top Verbraucher' }); stats.consumers.forEach(c => entities.push({ entity: c.entity_id, secondary_info: `${c.value.toFixed(2)} ${c.unit}` })); }
      return { type: 'entities', title: 'Energieübersicht', icon: 'mdi:lightning-bolt', entities, ...options };
    },
    buildClimateCard(stats, options = {}) {
      const entities = [{ type: 'custom:mushroom-chips-card', chips: [{ type: 'template', icon: 'mdi:thermometer', content: `⌀ ${stats.avgTemperature}°C`, icon_color: this.getTemperatureColor(parseFloat(stats.avgTemperature)) }, { type: 'template', icon: 'mdi:water-percent', content: `⌀ ${stats.avgHumidity}%`, icon_color: this.getHumidityColor(parseFloat(stats.avgHumidity)) }] }];
      if (stats.roomTemps.length) { entities.push({ type: 'section', label: 'Räume' }); stats.roomTemps.slice(0, 5).forEach(r => entities.push({ type: 'custom:mushroom-entity-card', name: r.area, icon: 'mdi:home-thermometer', primary_info: 'name', secondary_info: `${r.value.toFixed(1)}${r.unit}` })); }
    ...
    buildSystemHealthCard(stats, options = {}) {
      const entities = [{ type: 'custom:mushroom-template-card', primary: 'Systemgesundheit', secondary: `${stats.healthPercentage}% verfügbar`, icon: 'mdi:heart-pulse', icon_color: stats.healthPercentage > 95 ? 'green' : stats.healthPercentage > 85 ? 'orange' : 'red', layout: 'horizontal' }, { type: 'section', label: 'Statistiken' }, { type: 'attribute', entity: 'sun.sun', attribute: 'elevation', name: 'Entitäten', format: 'total', suffix: ` ${stats.totalEntities}`, icon: 'mdi:label' }, { type: 'attribute', entity: 'sun.sun', attribute: 'elevation', name: 'Geräte', format: 'total', suffix: ` ${stats.totalDevices}`, icon: 'mdi:devices' }];
      ...
      entities.push({ type: 'section', label: 'Automatisierungen' }, { type: 'attribute', entity: 'sun.sun', attribute: 'elevation', name: 'Aktiv', format: 'total', suffix: ` ${stats.automations.active} / ${stats.automations.total}`, icon: 'mdi:robot' });
      if (stats.updatesAvailable > 0) { entities.push({ type: 'section', label: `Updates verfügbar (${stats.updatesAvailable})` }); stats.updateEntities.slice(0, 3).forEach(u => entities.push({ entity: u.entity_id, secondary_info: u.version })); }
  ...
  window.L30NEYNThemeManager = {
    ...
    init(config = {}) {
      this.currentTheme = { mode: config.theme_mode || this._load('theme_mode') || 'auto', colorScheme: config.color_scheme || this._load('color_scheme') || 'default' };
      this.applyTheme();
    ...
    setMode(mode) { if (!['auto','light','dark'].includes(mode)) return; this.currentTheme.mode = mode; this._save('theme_mode', mode); this.applyTheme(); if (mode === 'auto') this._watchSystem(); },
    ...
    getAvailableColorSchemes() { return [{ id: 'default', name: 'Standard', icon: 'mdi:palette' }, { id: 'blue', name: 'Blau', icon: 'mdi:palette', color: '#2196f3' }, { id: 'green', name: 'Grün', icon: 'mdi:palette', color: '#4caf50' }, { id: 'purple', name: 'Lila', icon: 'mdi:palette', color: '#9c27b0' }, { id: 'red', name: 'Rot', icon: 'mdi:palette', color: '#f44336' }, { id: 'orange', name: 'Orange', icon: 'mdi:palette', color: '#ff9800' }, { id: 'teal', name: 'Türkis', icon: 'mdi:palette', color: '#009688' }]; },
    ...
    _watchSystem() { const mq = window.matchMedia('(prefers-color-scheme: dark)'); if (this._mql) { try { mq.removeEventListener('change', this._mql); } catch(e) {} } this._mql = () => { if (this.currentTheme.mode === 'auto') this.applyTheme(); }; mq.addEventListener('change', this._mql); },
  };

  // ─── CONFIG MANAGER ───────────────────────────────────────────────────────

  window.L30NEYNConfigManager = {
    defaultConfig: { theme_mode: 'auto', color_scheme: 'default', show_welcome: true, show_areas: true, show_security: true, show_light_summary: true, show_battery_status: true, weather_entity: null, show_energy_stats: true, show_climate_stats: true, show_system_health: true, show_network_stats: false, debug_mode: false, areas_options: {} },
  ...
  window.L30NEYNOverviewView = {
    generate(hass, config, registry) {
      ...
      const cards = [];
      if (config.show_welcome !== false) cards.push({ type: 'markdown', content: `# Willkommen\n\n${this._greeting()}` });
      ...
      if (config.show_security !== false) {
        ...
        if (sec.locks.length || sec.doors.length || sec.windows.length || sec.alarm) cards.push(builders.buildSecurityCard(sec));
      }
      if (config.show_light_summary !== false) {
        const lights = collectors.collectLights(hass, entities, config);
        cards.push({ type: 'entities', title: 'Beleuchtung', entities: [{ type: 'attribute', entity: 'sun.sun', attribute: 'next_rising', name: 'Lichter an', format: 'total', suffix: ` (${lights.on.length})` }, { type: 'attribute', entity: 'sun.sun', attribute: 'next_setting', name: 'Lichter aus', format: 'total', suffix: ` (${lights.off.length})` }] });
      }
      if (config.show_battery_status !== false) {
        const bats = collectors.collectBatteries(hass, entities, config);
        if (bats.critical.length || bats.low.length) cards.push(builders.buildBatteryCard(bats.critical, bats.low));
      }
    ...
    _areaCards(areas, hass, entities, devices, config) {
      const h = window.L30NEYNHelpers;
      const deviceAreaMap = h.buildDeviceAreaMap(devices);
      return areas.filter(a => !a.labels || !a.labels.includes('no_dboard')).map(area => {
        ...
        return { type: 'button', name: area.name, icon: area.icon || 'mdi:home', tap_action: { action: 'navigate', navigation_path: `/lovelace/${area.area_id}` }, entity: lights[0]?.entity_id };
      });
    },
    _greeting() { const h = new Date().getHours(); if (h < 6) return 'Gute Nacht'; if (h < 12) return 'Guten Morgen'; if (h < 18) return 'Guten Tag'; return 'Guten Abend'; },
  };
  ...
  window.L30NEYNRoomView = {
    ...
    DOMAIN_TITLES: { light: 'Beleuchtung', cover: 'Rollos & Vorhänge', climate: 'Klima', fan: 'Ventilatoren', switch: 'Schalter', media_player: 'Medien', sensor: 'Sensoren', binary_sensor: 'Status', camera: 'Kameras' },
    generate(areaId, hass, config, registry) {
      const { entities, devices, areas } = registry;
      const area = areas.find(a => a.area_id === areaId);
      if (!area) return { title: 'Fehler', path: areaId, cards: [{ type: 'markdown', content: 'Raum nicht gefunden.' }] };
      const collectors = window.L30NEYNDataCollectors;
      const builders = window.L30NEYNCardBuilders;
      ...
      for (const domain of this.DOMAIN_ORDER) { if (!roomEntities[domain] || !roomEntities[domain].length) continue; const c = this._domainCard(domain, roomEntities[domain], hass, config); if (c) cards.push(c); }
      for (const domain in roomEntities) { if (this.DOMAIN_ORDER.includes(domain)) continue; const c = this._domainCard(domain, roomEntities[domain], hass, config); if (c) cards.push(c); }
      return { title: area.name, path: areaId, icon: area.icon || 'mdi:home', cards: cards.length ? cards : [{ type: 'markdown', content: 'Keine Geräte in diesem Raum.' }] };
    },
    _domainCard(domain, entityIds, hass, config) {
      const builders = window.L30NEYNCardBuilders;
      const title = this.DOMAIN_TITLES[domain] || domain.replace('_', ' ');
      if (domain === 'sensor' || domain === 'binary_sensor') {
        const relevant = entityIds.filter(id => { const s = hass.states[id]; if (!s) return false; return ['temperature','humidity','illuminance','motion','occupancy','door','window'].includes(s.attributes.device_class); });
  ...
  window.L30NEYNStatisticsView = {
    generate(hass, config, registry) {
      ...
      if (config.show_energy_stats !== false) { const c = builders.buildEnergyCard(collectors.collectEnergy(hass, entities, config)); if (c) cards.push(c); }
      if (config.show_climate_stats !== false) { const c = builders.buildClimateCard(collectors.collectClimate(hass, entities, areas, config)); if (c) cards.push(c); }
      if (config.show_system_health !== false) { const c = builders.buildSystemHealthCard(collectors.collectSystemHealth(hass, entities, config)); if (c) cards.push(c); }
      if (config.show_network_stats === true) { const c = builders.buildNetworkCard(collectors.collectNetwork(hass, entities, config)); if (c) cards.push(c); }
  ...
  window.L30NEYNSettingsView = {
    generate(hass, config, registry) {
      const cards = [
        ...
        { type: 'entities', title: 'Design & Theme', icon: 'mdi:palette', entities: [{ type: 'custom:mushroom-select-card', entity: 'input_select.ha_custom_theme_mode', name: 'Theme-Modus', icon: 'mdi:theme-light-dark' }, { type: 'custom:mushroom-select-card', entity: 'input_select.ha_custom_color_scheme', name: 'Farbschema', icon: 'mdi:palette' }] },
        { type: 'entities', title: 'Übersichtsseite', icon: 'mdi:home', entities: [{ entity: 'input_boolean.ha_custom_show_welcome', name: 'Begrüßung anzeigen' }, { entity: 'input_boolean.ha_custom_show_areas', name: 'Raumübersicht anzeigen' }, { entity: 'input_boolean.ha_custom_show_security', name: 'Sicherheitsstatus anzeigen' }, { entity: 'input_boolean.ha_custom_show_light_summary', name: 'Licht-Zusammenfassung anzeigen' }, { entity: 'input_boolean.ha_custom_show_battery_status', name: 'Batteriestatus anzeigen' }] },
        { type: 'entities', title: 'Statistiken', icon: 'mdi:chart-line', entities: [{ entity: 'input_boolean.ha_custom_show_energy_stats', name: 'Energie-Statistiken' }, { entity: 'input_boolean.ha_custom_show_climate_stats', name: 'Klima-Statistiken' }, { entity: 'input_boolean.ha_custom_show_system_health', name: 'System-Gesundheit' }, { entity: 'input_boolean.ha_custom_show_network_stats', name: 'Netzwerk-Statistiken' }] },
        this._areaConfigCard(registry.areas, config),
        { type: 'entities', title: 'Erweitert', icon: 'mdi:cog-outline', entities: [{ entity: 'input_boolean.ha_custom_debug_mode', name: 'Debug-Modus', icon: 'mdi:bug' }] },
      ];
    ...
    _areaConfigCard(areas, config) {
      ...
      areas.filter(a => !a.labels || !a.labels.includes('no_dboard')).forEach(a => entities.push({ type: 'custom:mushroom-entity-card', name: a.name, icon: a.icon || 'mdi:home', tap_action: { action: 'navigate', navigation_path: `/lovelace/settings-area-${a.area_id}` }, icon_color: 'blue' }));
    ...
    generateAreaSettings(areaId, hass, config, registry) {
      ...
      ['light','cover','climate','switch','sensor'].forEach(domain => {
        ...
        cards.push({ type: 'entities', title: titles[domain] || domain, entities: [{ entity: `input_boolean.ha_custom_${areaId}_${domain}_enabled`, name: 'Anzeigen' }] });
      });
  ...
  class L30NEYNDashboardStrategy {
    static async generateDashboard(info) {
      ...
      if (window.L30NEYNThemeManager) window.L30NEYNThemeManager.applyTheme(hass, mergedConfig);
    ...
    static async generateView(info) {
      ...
      const mergedConfig = { ...config, ...strategyConfig };
      const registry = await this._fetchRegistries(hass);
      if (view.path === 'overview') return window.L30NEYNOverviewView.generate(hass, mergedConfig, registry);
      if (view.path === 'settings') return window.L30NEYNSettingsView.generate(hass, mergedConfig, registry);
      if (view.path === 'statistics') return window.L30NEYNStatisticsView.generate(hass, mergedConfig, registry);
      if (view.path.startsWith('settings-area-')) return window.L30NEYNSettingsView.generateAreaSettings(view.path.replace('settings-area-', ''), hass, mergedConfig, registry);
    ...
    static async _generateViews(hass, config, registry) {
      ...
      if (window.L30NEYNStatisticsView) views.push(window.L30NEYNStatisticsView.generate(hass, config, registry));
      for (const area of registry.areas) {
        if (area.labels && area.labels.includes('no_dboard')) continue;
        views.push(window.L30NEYNRoomView.generate(area.area_id, hass, config, registry));
      }
      if (window.L30NEYNSettingsView) views.push(window.L30NEYNSettingsView.generate(hass, config, registry));
      return views;
    }
  }
